...
def renderDepth(view, scene, maxDepth):
	for y in range(width):
		...
		for x in range(width):
			...
			if result is not -1:
				depth = int(255 - min(result[1] / maxDepth * 255, 255))
				pen.point((x, y), (depth, depth, depth, 255))

def renderNormal(view, scene):
	for y in range(width):
		sy = 1 - y / width
		for x in range(width):
			sx = x / width
			...
			result = scene.intersect(ray)
			if result is not -1:
				pen.point((x, y), getColor(result[3]))
...
# sphere = Sphere(numpy.array((0, 0, -10)), 10)

# renderDepth(view, sphere, 20)
# renderNormal(view, sphere)
